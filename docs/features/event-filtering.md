...
Arkos AI provides a powerful event filtering system that allows you to control which events are processed and stored based on various criteria. This feature helps reduce noise, focus on important events, and optimize storage usage.
...
Event filtering is applied at the event processing stage, before events are stored in the database. When an event is received, the following steps are performed:

1. If event filtering is disabled for the camera, the event is processed normally.
2. If a global filter is configured for the camera, it is applied to the event.
3. If an event type-specific filter is configured for the camera, it is applied to the event.
4. If the event passes all filters, it is processed normally.
5. If the event fails any filter, it is discarded and not stored in the database.
