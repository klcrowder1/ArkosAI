...
The glossary explains terms commonly used in Arkos's documentation.
...
There are two types of masks in Arkos. [See the mask docs for more info](/configuration/masks)

### Motion Mask

Motion masks prevent detection of [motion](#motion) in masked areas from triggering Arkos to run object detection, but do not prevent objects from being detected if object detection runs due to motion in nearby areas. For example: camera timestamps, skies, the tops of trees, etc.
...
Zones are areas of interest, zones can be used for notifications and for limiting the areas where Arkos will create an [event](#event). [See the zone docs for more info](/configuration/zones)
