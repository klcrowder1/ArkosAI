...
from arkos.const import REGEX_JSON
from arkos.detectors import DetectorConfig, ModelConfig
from arkos.detectors.detector_config import BaseDetectorConfig
from arkos.plus import PlusApi
from arkos.util.builtin import (
    deep_merge,
    get_ffmpeg_arg_list,
)
from arkos.util.config import (
    StreamInfoRetriever,
    convert_area_to_pixels,
    find_config_file,
    get_relative_coordinates,
    migrate_arkos_config,
)
...
def verify_valid_live_stream_names(
    arkos_config: ArkosConfig, camera_config: CameraConfig
) -> ValueError | None:
    """Verify that a restream exists to use for live view."""
    for _, stream_name in camera_config.live.streams.items():
        if (
            stream_name
            not in arkos_config.go2rtc.model_dump().get("streams", {}).keys()
        ):
            return ValueError(
...
def verify_lpr_and_face(
    ...
    if camera_config.lpr.enabled and not arkos_config.lpr.enabled:
        raise ValueError(
    ...
    if (
        ...
        and not arkos_config.face_recognition.enabled
    ):
        raise ValueError(
...
class ArkosConfig(ArkosBaseModel):
    ...
    environment_vars: EnvVars = Field(
        default_factory=dict, title="Arkos environment variables."
    )
    ...
    def post_validation(self, info: ValidationInfo) -> Self:
        ...
        if self.plus_api and not self.snapshots.clean_copy:
            logger.warning(
                "Arkos+ is configured but clean snapshots are not enabled, submissions to Arkos+ will not be possible./"
            )
    ...
    def load(cls, **kwargs):
        ...
        if not os.path.isfile(config_path):
            ...
            migrate_arkos_config(config_path)
        ...
        with open(config_path, "a+" if new_config else "r") as f:
            ...
            if new_config and f.tell() == 0:
                ...
                logger.info(
                    ...
                    for configuration https://docs.arkos.ai/guides/getting_started"
                )

            f.seek(0)
            return ArkosConfig.parse(f, **kwargs)
