...
from arkos.camera import PTZMetrics
from arkos.config import CameraConfig
from arkos.ptz.autotrack import PtzMotionEstimator
...
from arkos.util.object import average_boxes, median_of_boxes
...
# Enhanced distance function that normalizes distance from estimate relative to object size
# and considers appearance similarity when available
def distance(detection: np.array, estimate: np.array, detection_embedding=None, tracked_object_embedding=None) -> float:
    ...
    distance[0] /= max(1.0, estimate_dim[0])
    # Change in y relative to h
    distance[1] /= max(1.0, estimate_dim[1])
    ...
    width_ratio = widths[1] / max(1.0, widths[0]) - 1.0
    height_ratio = heights[1] / max(1.0, heights[0]) - 1.0
    ...
    spatial_distance = np.linalg.norm(change)
    
    # If embeddings are available, incorporate appearance similarity
    if detection_embedding is not None and tracked_object_embedding is not None:
        appearance_distance = 1 - cv2.compareHist(
            detection_embedding, tracked_object_embedding, cv2.HISTCMP_CORREL
        )
        # Weighted combination of spatial and appearance distance
        # Weight can be adjusted based on the reliability of each component
        return 0.7 * spatial_distance + 0.3 * appearance_distance
...
def frigate_distance(detection: Detection, tracked_object) -> float:
    ...
    if hasattr(tracked_object, 'last_detection') and tracked_object.last_detection is not None:
        tracked_object_embedding = tracked_object.last_detection.embedding
    
    return distance(detection.points, tracked_object.estimate, detection_embedding, tracked_object_embedding)
...
class NorfairTracker(ObjectTracker):
    def __init__(
        ...
        self.track_id_map = {}
        self.object_confidence = {}  # Track confidence for each object

        # Define enhanced tracker configurations for static camera
        self.object_type_configs = {
            ...
            "person": {
                ...
                "reid_distance_threshold": 0.45,  # Slightly more strict
                "reid_hit_counter_max": 15,  # Increased for better persistence
            },
            "license_plate": {
        ...
        # Define enhanced autotracking PTZ-specific configurations
    ...
    def register(self, track_id, obj):
        ...
        self.object_confidence[id] = 1.0  # Initialize with high confidence
    ...
    def deregister(self, id, track_id):
        ...
        logger.debug(f"Deregistering object: {id}, label: {obj['label']}")

        del self.tracked_objects[id]
        del self.disappeared[id]
        del self.object_confidence[id]
        ...
        if id in self.stationary_box_history:
            del self.stationary_box_history[id]
    ...
    def update_position(self, id: str, box: list[int, int, int, int], stationary: bool):
        ...
        if avg_iou < THRESHOLD_KNOWN_ACTIVE_IOU:
            ...
            # Reduce confidence when position changes significantly
            self.object_confidence[id] = max(0.7, self.object_confidence[id] - 0.1)
            return False
        ...
        if avg_iou < threshold:
            ...
            if median_iou < threshold:
                ...
                # Reduce confidence when position changes
                self.object_confidence[id] = max(0.7, self.object_confidence[id] - 0.1)
                return False
        ...
        if 5 <= len(position["xmins"]) < 10:
            ...
            self.object_confidence[id] = min(1.0, self.object_confidence[id] + 0.05)
    ...
    def update(self, track_id, obj):
        ...
        obj["confidence"] = self.object_confidence[id]
    ...
    def match_and_update(
        ...
        for t in all_tracked_objects:
            ...
            # if there wasn't a detection in this frame, increment disappeared
                ...
                self.object_confidence[id] = max(0.1, self.object_confidence[id] - 0.1)
    ...
    def debug_draw(self, frame, frame_time):
        ...
        for obj in all_tracked_objects:
            ...
            if obj.global_id in self.track_id_map:
                id = self.track_id_map[obj.global_id]
                if id in self.object_confidence:
                    confidence_text_anchor = (
                        ld.points[1, 0],
                        ld.points[1, 1] + 20,  # Position below the distance text
                    )
                    frame = Drawer.text(
                        frame,
                        f"Conf: {self.object_confidence[id]:.2f}",
                        position=confidence_text_anchor,
                        size=None,
                        color=(0, 255, 0),
                        thickness=None,
                    )
