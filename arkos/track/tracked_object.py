...
from arkos.config import (
    CameraConfig,
    ModelConfig,
    SnapshotsConfig,
    UIConfig,
)
from arkos.const import CLIPS_DIR, THUMB_DIR
from arkos.review.types import SeverityEnum
from arkos.util.image import (
    area,
    calculate_region,
    draw_box_with_label,
    draw_timestamp,
...
from arkos.util.object import box_inside
from arkos.util.velocity import calculate_real_world_speed
...
class TrackedObject:
    def __init__(
        ...
        self.path_data = []
        self.confidence_history = []  # Track confidence history
    ...
    def _is_false_positive(self):
        ...
        # If confidence is available, use it to adjust the threshold
        if "confidence" in self.obj_data:
            # Adjust threshold based on confidence (lower threshold for high confidence)
            adjusted_threshold = threshold * (1.0 - 0.2 * self.obj_data["confidence"])
            return self.computed_score < adjusted_threshold
        
        return self.computed_score < threshold

    def compute_score(self):
        """Get median of scores for object with confidence weighting."""
        if len(self.score_history) == 0:
            return 0.0
            
        # If confidence is available, use it to weight the scores
        if "confidence" in self.obj_data:
            # Apply confidence as a multiplier to the median score
            # This gives more weight to objects with high tracking confidence
            confidence_factor = 0.8 + (0.2 * self.obj_data["confidence"])
            return median(self.score_history) * confidence_factor
    ...
    def update(self, current_frame_time: float, obj_data, has_valid_frame: bool):
        ...
        if "confidence" in obj_data:
            self.confidence_history.append(obj_data["confidence"])
            ...
            if self.thumbnail_data is None or is_better_thumbnail(
                ...
                if obj_data["frame_time"] == current_frame_time:
                    self.thumbnail_data = {
                        ...
                        "confidence": obj_data.get("confidence", 1.0),  # Include confidence
                    }
                    thumb_update = True
                else:
                    logger.debug(
                        ...
                        if loitering_score >= (
                            ...
                            * self.camera_config.detect.fps
                            ...
                            if name not in self.entered_zones:
                                ...
                                # Zone entry is a significant change
        ...
        for attr in obj_data["attributes"]:
            if self.attributes[attr["label"]] < attr["score"]:
                ...
                # New attribute or higher score is a significant change
                significant_change = True
        ...
        if self.obj_data["label"] in ["car", "motorcycle", "package", "person"]:
            ...
            if len(recognized_logos) > 0:
                ...
                if (
                    ...
                    # New sub_label is a significant change
                    if self.obj_data.get("sub_label") is None:
                        significant_change = True
        ...
        if not self.false_positive:
            ...
            # Use bottom center for better tracking of people and objects on the ground
            bottom_center = (
                round(obj_data["centroid"][0] / width, 4),
                round(obj_data["box"][3] / height, 4),
            )

            # Adaptive threshold based on object size and movement speed
            # Smaller threshold for small objects, larger for big objects
            obj_size = (obj_data["box"][2] - obj_data["box"][0]) * (obj_data["box"][3] - obj_data["box"][1])
            size_factor = obj_size / (width * height)
            
            # Base threshold is 5% of the frame diagonal, adjusted by object size
            base_threshold = 0.05 * math.sqrt(width**2 + height**2) / max(width, height)
            adaptive_threshold = base_threshold * (0.5 + size_factor * 5)
            
            # Further adjust by confidence if available
            if "confidence" in obj_data:
                # Higher confidence = lower threshold (more precise tracking)
                confidence_factor = 1.0 - (0.5 * obj_data["confidence"])
                adaptive_threshold *= confidence_factor
            
            # Minimum threshold to avoid noise
            adaptive_threshold = max(0.01, adaptive_threshold)

            if not self.path_data:
                self.path_data.append((bottom_center, obj_data["frame_time"]))
                path_update = True
            elif (
                math.dist(self.path_data[-1][0], bottom_center) >= adaptive_threshold
                ...
                path_update = True
                logger.debug(
                    f"Point tracking: {obj_data['id']}, {bottom_center}, {obj_data['frame_time']}, threshold: {adaptive_threshold:.4f}"
                )
                
            # Limit path data to prevent memory issues
            if len(self.path_data) > 100:
                # Keep first few points and most recent points
                self.path_data = self.path_data[:5] + self.path_data[-95:]
    ...
    def to_dict(self):
        ...
        event = {
            ...
            "confidence": self.obj_data.get("confidence"),
    ...
    def get_img_bytes(
        ...
        if bounding_box:
            ...
            # Include confidence in the label if available
            confidence_text = ""
            if "confidence" in self.thumbnail_data:
                confidence_text = f" Conf:{self.thumbnail_data['confidence']:.2f}"
            ...
            # Draw path if available and object is not stationary
            if self.path_data and len(self.path_data) > 1 and self.active:
                frame_h, frame_w = best_frame.shape[:2]
                points = []
                
                # Convert normalized coordinates to pixel coordinates
                for point, _ in self.path_data:
                    x = int(point[0] * frame_w)
                    y = int(point[1] * frame_h)
                    points.append((x, y))
                
                # Draw path as connected line segments
                for i in range(1, len(points)):
                    # Use color gradient from green to red to show direction
                    ...
                    cv2.line(best_frame, points[i-1], points[i], path_color, 2)
                
                # Draw dots at each point
                for i, point in enumerate(points):
                    # First point is green, last point is red
                    if i == 0:
                        cv2.circle(best_frame, point, 3, (0, 255, 0), -1)
                    elif i == len(points) - 1:
                        cv2.circle(best_frame, point, 3, (0, 0, 255), -1)
                    else:
                        cv2.circle(best_frame, point, 2, (255, 255, 255), -1)
...
class TrackedObjectAttribute:
    ...
    def find_best_object(self, objects: list[dict[str, any]]) -> Optional[str]:
        ...
        for obj in objects:
            ...
            object_confidence = obj.get("confidence", 0.0)
            ...
            if best_object_area is None:
                ...
                best_object_confidence = object_confidence
            else:
                if best_object_label == "car" and obj["label"] == "car":
                    # If both are cars, prefer the one with higher confidence
                    if object_confidence > best_object_confidence:
                        best_object_area = object_area
                        best_object_id = obj["id"]
                        best_object_label = obj["label"]
                        best_object_confidence = object_confidence
