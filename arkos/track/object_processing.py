...
from arkos.camera.state import CameraState
from arkos.comms.config_updater import ConfigSubscriber
from arkos.comms.detections_updater import DetectionPublisher, DetectionTypeEnum
from arkos.comms.dispatcher import Dispatcher
from arkos.comms.event_metadata_updater import (
    EventMetadataSubscriber,
    EventMetadataTypeEnum,
)
from arkos.comms.events_updater import EventEndSubscriber, EventUpdatePublisher
from arkos.comms.inter_process import InterProcessRequestor
...
from arkos.const import FAST_QUEUE_TIMEOUT, UPDATE_CAMERA_ACTIVITY
from arkos.events.types import EventStateEnum, EventTypeEnum
from arkos.models import Event, Timeline
from arkos.track.tracked_object import TrackedObject
from arkos.util.image import SharedMemoryFrameManager
...
class TrackedObjectProcessor(threading.Thread):
    ...
    def should_save_snapshot(self, camera, obj: TrackedObject):
        ...
        # Consider confidence score if available
        if "confidence" in obj.obj_data:
            # Only save snapshots for objects with confidence above threshold
            if obj.obj_data["confidence"] < 0.5:  # Configurable threshold
                logger.debug(
                    f"Not creating snapshot for {obj.obj_data['id']} because confidence {obj.obj_data['confidence']} is below threshold"
                )
                return False
    ...
    def should_retain_recording(self, camera: str, obj: TrackedObject):
        ...
        # Consider confidence score if available
        if "confidence" in obj.obj_data:
            # Only retain recordings for objects with confidence above threshold
            if obj.obj_data["confidence"] < 0.6:  # Higher threshold for recordings
                logger.debug(
                    f"Not retaining recording for {obj.obj_data['id']} because confidence {obj.obj_data['confidence']} is below threshold"
                )
                return False
    ...
    def should_mqtt_snapshot(self, camera, obj: TrackedObject):
        ...
        # Consider confidence score if available
        if "confidence" in obj.obj_data:
            # Only send MQTT for objects with confidence above threshold
            if obj.obj_data["confidence"] < 0.5:  # Configurable threshold
                logger.debug(
                    f"Not sending MQTT for {obj.obj_data['id']} because confidence {obj.obj_data['confidence']} is below threshold"
                )
                return False
    ...
    def log_processing_stats(self):
        ...
        time_since_last_report = current_time - self.processing_stats["last_report_time"]
        
        # Log stats every 5 minutes
        if time_since_last_report >= 300:
            frames = self.processing_stats["processed_frames"]
            objects = self.processing_stats["processed_objects"]
            
            fps = frames / elapsed_time if elapsed_time > 0 else 0
            objects_per_second = objects / elapsed_time if elapsed_time > 0 else 0
            
            logger.info(
                f"Processing stats: {frames} frames ({fps:.2f} fps), "
                f"{objects} objects ({objects_per_second:.2f} obj/s) "
                f"over {elapsed_time:.1f} seconds"
            )
            
            self.processing_stats["last_report_time"] = current_time
    ...
    def run(self):
        while not self.stop_event.is_set():
            ...
            self.processing_stats["processed_objects"] += len(current_tracked_objects)
            self.log_processing_stats()

            # Process batch of objects more efficiently
            start_time = time.time()
            camera_state.update(
            ...
            if processing_time > 0.1:  # More than 100ms
                logger.debug(
                    f"Camera {camera}: Processing {len(current_tracked_objects)} objects took {processing_time:.3f}s"
                )
