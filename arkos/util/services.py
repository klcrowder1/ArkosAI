...
from arkos.util.builtin import clean_camera_user_pass, escape_special_characters
...
def restart_arkos():
    ...
    if proc.name() == "s6-svscan":
        proc.terminate()
    # otherwise, just try and exit arkos
    else:
...
def get_bandwidth_stats(config) -> dict[str, dict]:
    ...
    if p.returncode != 0:
        ...
        for line in lines:
            ...
            try:
                if re.search(
                    r"(^ffmpeg|\/go2rtc|arkos\.detector\.[a-z]+)/([0-9]+)/", stats[0]
                ):
...
def process_logs(
    ...
    for raw_line in contents.splitlines():
        ...
        try:
            ...
            # For arkos, remove the date part from message comparison
            if service == "arkos":
