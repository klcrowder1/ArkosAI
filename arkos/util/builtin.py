...
from arkos.const import REGEX_HTTP_CAMERA_USER_PASS, REGEX_RTSP_CAMERA_USER_PASS
...
def update_yaml_file(file_path, key_path, new_value):
    ...
    try:
        ...
        logger.error(
            f"Unable to read from Arkos config file {file_path}. Make sure it exists and is readable."
        )
        return
    ...
    try:
        ...
        logger.error(f"Unable to write to Arkos config file {file_path}: {e}")
...
def clear_and_unlink(file: Path, missing_ok: bool = True) -> None:
    ...
    # empty contents of file before unlinking https://github.com/ArkosAI/ArkosAI/issues/4769
    with open(file, "w"):
        pass
