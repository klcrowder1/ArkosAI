...
from arkos.const import DEFAULT_ATTRIBUTE_LABEL_MAP, MODEL_CACHE_DIR
from arkos.util.builtin import generate_color_palette, load_labels
...
class ModelConfig(BaseModel):
    ...
    # New field for model purpose/role
    purpose: str = Field(
        default="general", 
        title="Purpose of this model (e.g., general, person, vehicle, face)"
    )
    # New field for model priority (lower number = higher priority)
    priority: int = Field(
        default=100, 
        title="Priority of this model (lower number = higher priority)"
    )
    ...
    def check_and_load_plus_model(
        self, plus_api, detector: str = None
    ) -> None:
...
class BaseDetectorConfig(BaseModel):
    ...
    # Changed from Optional[ModelConfig] to List[ModelConfig] to support multiple models
    models: List[ModelConfig] = Field(
        default_factory=list, title="List of models for this detector."
    )
    # For backward compatibility
    model: Optional[ModelConfig] = Field(
        default=None, title="Detector specific model configuration (legacy)."
    )
    model_path: Optional[str] = Field(
        default=None, title="Detector specific model path (legacy)."
    )
    ...
    def __init__(self, **data):
        super().__init__(**data)
        # Handle backward compatibility - if model is provided but models is empty,
