...
from arkos.comms.event_metadata_updater import EventMetadataPublisher
from arkos.comms.inter_process import InterProcessCommunicator
from arkos.comms.mqtt import MqttClient
from arkos.comms.webpush import WebPushClient
from arkos.comms.ws import WebSocketClient
from arkos.comms.zmq_proxy import ZmqProxy
from arkos.config.config import ArkosConfig
from arkos.const import (
    CACHE_DIR,
    CLIPS_DIR,
    CONFIG_DIR,
    EXPORT_DIR,
...
from arkos.db.sqlitevecq import SqliteVecQueueDatabase
...
from arkos.output.output import output_frames
from arkos.ptz.autotrack import PtzAutoTrackerThread
from arkos.ptz.onvif import OnvifController
from arkos.record.cleanup import RecordingCleanup
from arkos.record.export import migrate_exports
from arkos.record.record import manage_recordings
...
from arkos.util.object import get_camera_regions_grid
from arkos.version import VERSION
from arkos.video import capture_camera, track_camera
from arkos.watchdog import ArkosWatchdog

logger = logging.getLogger(__name__)


class ArkosApp:
    def __init__(self, config: ArkosConfig) -> None:
    ...
    def start_watchdog(self) -> None:
        self.arkos_watchdog = ArkosWatchdog(self.detectors, self.stop_event)
        self.arkos_watchdog.start()
    ...
    def start(self) -> None:
        logger.info(f"Starting Arkos ({VERSION})")

        # Ensure global state.
        self.ensure_dirs()

        # Start arkos services.
