...
from arkos.config import FfmpegConfig, ArkosConfig
...
from arkos.ffmpeg_presets import (
    EncodeTypeEnum,
    parse_preset_hardware_acceleration_encode,
)
from arkos.models import Export, Previews, Recordings
from arkos.util.builtin import is_current_hour
...
class RecordingExporter(threading.Thread):
    ...
    def get_record_export_command(self, video_path: str) -> list[str]:
        ...
        # add metadata
        title = f"Arkos Recording for {self.camera}, {self.get_datetime_from_timestamp(self.start_time)} - {self.get_datetime_from_timestamp(self.end_time)}"
    ...
    def get_preview_export_command(self, video_path: str) -> list[str]:
        ...
        # add metadata
        title = f"Arkos Preview for {self.camera}, {self.get_datetime_from_timestamp(self.start_time)} - {self.get_datetime_from_timestamp(self.end_time)}"
        ffmpeg_cmd.extend(["-metadata", f"title={title}"])
