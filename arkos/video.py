...
from arkos.config import CameraConfig, DetectConfig, ModelConfig
from arkos.config.camera.camera import CameraTypeEnum
...
from arkos.motion import MotionDetector
from arkos.motion.improved_motion import ImprovedMotionDetector
from arkos.object_detection.base import RemoteObjectDetector
from arkos.ptz.autotrack import ptz_moving_at_frame_time
from arkos.track import ObjectTracker
from arkos.track.norfair_tracker import NorfairTracker
from arkos.track.tracked_object import TrackedObjectAttribute
from arkos.util.builtin import EventsPerSecond, get_tomorrow_at_time
from arkos.util.image import (
    FrameManager,
    SharedMemoryFrameManager,
    draw_box_with_label,
)
from arkos.util.object import (
    create_tensor_input,
...
from arkos.util.services import listen
...
def track_camera(
    ...
    setproctitle(f"arkos.process:{name}")
    listen()
...
def process_frames(
    ...
    while not stop_event.is_set():
        ...
        if not detect_config.enabled:
            ...
            # else, just update the frame times for the stationary
